...
class AlarmsFragment : Fragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //viewModel.fakeNames.observe(this, Observer { response -> favourite_text.text = response.first()})
        viewModel.lastPlayedSong.asLiveData().observe(this) { response ->
            if (response.isNotEmpty())
                favourite_text.text = response.first()
        }
        arguments?.let {
